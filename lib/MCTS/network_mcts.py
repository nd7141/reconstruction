...
class AgentLSTM(nn.Module):
    def __init__(self, hid_size, gcn_size=None, vertex_emb_size=None,
        ...
        self.path_lstm = nn.LSTM(input_size=path_emb_size, hidden_size=hid_size, num_layers=lstm_size, batch_first=True)
        self.walk_out = nn.Linear(hid_size, hid_size)
    ...
    def embed_paths(self, paths, graph_embs, *, device=None, **kwargs):
        """
        :param walk: a tensor of anonymous vertex indices, shape: [batch, length]
        :return: matrix [batch, length, hid_size]
        """
        paths = paths.to(device=device)
        l = []
        for i in x:
            l.append(graph_emb[i])
        l = torch.stack(l).unsqueeze(0)
        out, (hn, cn) = self.path_lstm(l)  # [batch, len, 2 * lstm_size]
        paths_embs = self.walk_out(hn[:, -1, :])   # [batch, len, hid_size]
        return paths_embs

    def embed_path(self, path, **kwargs):
        return self.embed_paths(torch.tensor([path]), **kwargs)[0]
