...
class MCTS():
    ...
    def getActionProb(self, path, buffer, temp=1):
        """
        ...
        for i in range(self.numMCTSSims):
            self.search(path, buffer)

        counts = [self.Nsa[(path,vertex)] if (path,vertex) in self.Nsa else 0 for vertex in self.edges[path[-1]]]
    ...
    def search(self, path, buffer):
        ...
        for a in self.edges[path[-1]]:
            k = self.Vs[path].index(a)
        ...
        v = self.search(tuple(next_path), buffer)
